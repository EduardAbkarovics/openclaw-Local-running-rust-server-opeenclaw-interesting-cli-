...
tmux send-keys -t "$SESSION:0.1" \
    "source '$VENV' && cd '$PROJECT/python_llm' && python model_server.py" Enter
...
# Bal pane: Chat kliens (3mp várakozás a szerver indulásáig)
tmux select-pane -t "$SESSION:0.0"
tmux send-keys -t "$SESSION:0.0" \
    "source '$VENV' && cd '$PROJECT/python_llm' && sleep 3 && python chat_cli.py" Enter
